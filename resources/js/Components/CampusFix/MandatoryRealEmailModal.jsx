...
import { Mail, ShieldCheck, AlertCircle, Loader2, CheckCircle2, ArrowRight, Lock } from 'lucide-react';
...
export function MandatoryRealEmailModal({ user }) {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!trimmed) {
            setErrorMessage('Silakan masukkan alamat email asli Anda.');
            return;
        }
        ...
        if (trimmed.endsWith('@telunas.com') || trimmed.endsWith('@example.com') || trimmed.endsWith('@test.com')) {
            setErrorMessage('Harap gunakan alamat email asli (seperti Gmail atau email aktif perusahaan), bukan domain dummy.');
            return;
        }
        ...
        try {
            ...
            const msg = err.response?.data?.errors?.email?.[0] || 
                        ...
                        'Gagal memperbarui email. Pastikan email belum digunakan akun lain.';
            setErrorMessage(msg);
        } finally {
            setIsLoading(false);
        }
                    ...
                    <div className="flex items-center gap-3.5 mb-2">
                        ...
                        <div>
                            ...
                            <DialogTitle className="text-base font-bold text-white tracking-wide">
                                Hubungkan Alamat Email Asli
                            </DialogTitle>
                        </div>
                    </div>
                    <DialogDescription className="text-xs text-[#A19F8D] leading-relaxed">
                        Akun Anda saat ini masih menggunakan email dummy: <strong className="text-rose-300 font-mono">{user.email}</strong>.
                    </DialogDescription>
                </div>
                ...
                <div className="p-6 space-y-4">
                    ...
                    <div className="p-3.5 rounded-xl bg-[#242217] border border-[#3B3929] space-y-2 text-xs text-[#A19F8D]">
                        <div className="flex items-start gap-2.5">
                            ...
                            <span>Email asli digunakan untuk <strong>laporan bulanan otomatis</strong> dan <strong>notifikasi resmi resort</strong>.</span>
                        </div>
                        <div className="flex items-start gap-2.5">
                            <Lock className="h-4 w-4 text-emerald-400 shrink-0 mt-0.5" />
                            <span><strong>Password Anda tetap sama</strong> dan tidak akan berubah.</span>
                        </div>
                    </div>
                    ...
                    {isSuccess ? (
                        <div className="p-4 rounded-xl bg-emerald-950/60 border border-emerald-500/40 text-emerald-200 flex items-center gap-3 text-xs">
                            ...
                            <div>
                                <div className="font-bold text-emerald-300">Email Berhasil Dihubungkan!</div>
                                <div className="text-[11px] text-emerald-200/80 mt-0.5">Memuat ulang sesi dashboard Anda...</div>
                            </div>
                        </div>
                    ) : (
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            <div>
                                <label className="block text-xs font-semibold text-[#FAFAFA] mb-1.5">
                                    Masukkan Alamat Email Asli Anda:
                                </label>
                                <input
                                    type="email"
                                    required
                            ...
                            <Button
                                ...
                                className="w-full bg-[#C9AA71] hover:bg-[#B8985E] text-[#1C1B0E] font-bold py-2.5 text-xs rounded-xl shadow-lg transition-all flex items-center justify-center gap-2"
                            >
                                {isLoading ? (
                                    <>
                                    ...
                                    <>
                                        <span>Hubungkan Email &amp; Lanjutkan</span>
